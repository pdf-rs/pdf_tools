...
fn add_array(arr: &[TextDrawAdjusted], out: &mut String, info: &FontInfo) {
    // println!("p: {:?}", p);
    for p in arr.iter() {
        if let TextDrawAdjusted::Text(s) = p {
            add_string(&s.data, out, info);
        }
    }
}
    ...
    for op in &contents.operations {
        match *op {
            ...
            Op::SetTextMatrix { matrix } => {
                if (matrix.f - text_matrix.f).abs() > f32::EPSILON {
                    out.push('\n');
                } else {
                    out.push('\t');
                }
